...
else
{
	...
	if ((Get-CimInstance win32_bios).Manufacturer -match 'Dell')
	{
		...
		$a = "/configure -scheduleWeekly=Mon,16:43 -updateSeverity=recommended,critical -updatetype=driver,bios -updateDeviceCategory=network,storage,audio,video,input -scheduleAction=DownloadInstallAndNotify -autoSuspendBitLocker=enable -outputLog=$setlog -encryptedPassword=ARERCAKWP7Xed0o4eLqcrlUsrNi4KXYCsCw1T+QIuEVSapDLTlS9vIp1hxz9y96594Xdlv2yrI6oyWtg== -encryptionKey=MTm45CqF"
		...
		$c = "/applyUpdates -silent -outputLog=$outlog -encryptedPassword=ARERCAKWP7Xed0o4eLqcrlUsrNi4KXYCsCw1T+QIuEVSapDLTlS9vIp1hxz9y96594Xdlv2yrI6orhyWtg== -encryptionKey=MT!Tm45CqF"
